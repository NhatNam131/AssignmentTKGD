...
public class KhoanChiDAO implements Contant {
    ...
    public int insertKhoanChi(KhoanChi khoanChi){
        ...
        values.put("Name", khoanChi.getName());
        values.put("Type", khoanChi.getType());
        values.put("Money", khoanChi.getMoney());
        values.put("Date", khoanChi.getDate());
        values.put("Description", khoanChi.getDescription());
        try {
            if (db.insert(TABLE_KHOAN_CHI, null, values) == -1) {
                return -1;
            }
        } catch (Exception ex) {
            Log.e(TABLE_KHOAN_CHI, ex.toString());
        }
        return 1;
    }

    public int updateKhoanChi(String Id, String Name, String Type, String Money, String Date, String Description) {
        ContentValues values = new ContentValues();
        values.put("Id", Id);
        ...
        int result = db.update(TABLE_KHOAN_CHI, values, "Id=?", new String[]{Id});
        if (result == 0) {
            return -1;
        }
        return 1;
    ...
    public List<KhoanChi> getAllKhoanChi() {
        ...
        Cursor c = db.query(TABLE_KHOAN_CHI, null, null, null, null, null, null);
        c.moveToFirst();
    ...
    public int deleteKhoanThuByID(String maKhoanChi) {
        int result = db.delete(TABLE_KHOAN_CHI, "Id=?", new String[]{maKhoanChi});
        if (result == 0)
            return -1;
        return 1;
    }
